...
export default function LeaveApprovalDashboard({managerId = 'MTL1008'}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://ssit-leaves-backend.azurewebsites.net/api/leave/manager/${managerId}`);
  ...
  const handleApprove = async (id) => {
    try {
      await axios.put(`https://ssit-leaves-backend.azurewebsites.net/api/leave/approve/${id}`);
      const response = await axios.get(`https://ssit-leaves-backend.azurewebsites.net/api/leave/manager/${managerId}`);
  ...
  const handleReject = async () => {
    ...
    //const encodedReason = encodeURIComponent(rejectionReason);
      await axios.put(`https://ssit-leaves-backend.azurewebsites.net/api/leave/reject/${selectedLeaveId}/${rejectionReason}`);
      const response = await axios.get(`https://ssit-leaves-backend.azurewebsites.net/api/leave/manager/${managerId}`);
      // Update the status count directly
    setStatusCount((prevStatusCount) => ({
      ...prevStatusCount,
